...
class FileScore(BaseModel):
    ...
    score: float = Field(
        ..., description="Relevance score from 0.0 to 10.0, higher is more relevant"
    )
...
class Query(BaseModel):
    query: str = Field(
        ..., description="Query to use for similarity search in the vectorstore"
    )
...
class WindowsInternals(Agent):
    ...
    def _build(self):
        ...
        builder.add_node(self.NODES.user_refinement, self.user_refinement)

        builder.set_entry_point(self.NODES.collect)
        builder.add_edge(self.NODES.collect, self.NODES.rank)

        builder.add_conditional_edges(
            self.NODES.rank, self.refinement, [self.NODES.user_refinement, END]
        )

        builder.set_finish_point(self.NODES.user_refinement)

        self._graph = builder.compile()

    async def user_refinement(self, context: WindowsInternalsContext):
        context.state_info.node.append(self.NODES.user_refinement)
        user_docs = context.user_docs if context.user_docs else []

        while True:
            console.info("\nOptions: semantic | filename (press Enter to continue)")
            ...
            if user_input == "semantic":
                ...
                if query:
                    search_results = (
                        await VectorStore.file_info.asimilarity_search_with_score(
                            query, k=10
                        )
                    )
                ...
                if pattern:
                    try:
                        results = VectorStore.file_info.get(
                            where={"name": {"$eq": pattern}}, limit=10
                        )
            ...
            for idx, (doc, score) in enumerate(search_results):
                ...
                console.info(f"{idx + 1}. {name} (score: {score:.3f})")
            ...
            indices = (
                ...
                if selection == "all"
                    ...
                    if i.strip().isdigit()
                    and 0 <= int(i.strip()) - 1 < len(search_results)
                ]
            )
            ...
            for idx in indices:
                ...
                if not any(
                    ...
                    console.info(f"+ {doc.metadata.get('name', 'Unknown')}")

        user_ranked_docs = [
            (doc, score, 10.0) for doc, score in user_docs
        ]

        candidates = sorted(
            context.candidates.results + user_ranked_docs, key=lambda t: t[2], reverse=True
        )

        console.info(
            f"[*] Final ranking: {len(context.candidates.results) + len(user_ranked_docs)} documents ({len(user_ranked_docs)} user-selected)"
        )

        # Update context with user docs
        return {"user_docs": user_docs, "candidates": Candidates(query=context.query, results=candidates)}

    def refinement(
        self, context: WindowsInternalsContext, config: RunnableConfig
    ):
        if not config.get("configurable", {}).get("interrupt", False):
            return END
        return self.NODES.user_refinement
    ...
    async def collect(self, context: WindowsInternalsContext):
        ...
        query_chain = (
            self.prompt_template.collect | self.get_llm().with_structured_output(Query)
        )
        metadata = json.dumps(
            {
                k: v
                for k, v in context.cve_details.msrc_report.to_dict().items()
                if k != "products"
            }
        )
        ...
        docs = await VectorStore.file_info.asimilarity_search_with_score(
            result.query, k=10
        )  # TODO: add config
        ...
        if len(docs):
            console.info(
                f"[+] Found {len(docs)} potential candidates for {context.cve_details.cve} (limit: 10)"
            )
        else:
    ...
    async def rank(self, context: WindowsInternalsContext):
        ...
        chain = (
            self.prompt_template.rank
            | AgentModels.default_model.model.with_structured_output(FileScoreList)
        )

        files = "\n\n".join(
            f"name: {doc.metadata.get('name', '')}\n{doc.page_content}"
            for doc, _ in context.docs
        )
        metadata = json.dumps(
            {
                k: v
        ...
        result: FileScoreList = await chain.ainvoke(
            {"query": context.query, "metadata": metadata, "files": files}
        )

        score_map: dict[str, float] = {fs.file: fs.score for fs in result.files}
        ranked_docs = [
            (doc, score, score_map.get(doc.metadata.get("name"), 0.0))
            for doc, score in context.docs
        ]

        candidates = sorted(
            ranked_docs, key=lambda t: t[2], reverse=True
        )
        ...
        ranked_df = pl.DataFrame(
            [
                ...
                for doc, score, rank in candidates
            ]
        )
        console.info(f"\n{context.cve_details.cve} ranked candidates:\n{ranked_df}")

        return {
            "candidates": Candidates(query=context.query, results=candidates),
        }
