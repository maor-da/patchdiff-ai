...
Picks between two `make_nodes` implementations at build time:
...
`<primary>.<secondary_kb>.BinDiff`, `__funcs__/<ea>.c`) so the rest of
...
def build_re_graph(ctx: AppContext):
    """Build the RE subgraph for the configured IDA install."""
    ...
    builder.add_node(ReNodes.ANALYZE, analyze)
    # Diff + decompile is one node so the live BinDiff (sqlite3-backed,
    # not pickleable) never crosses a checkpoint boundary.
    builder.add_node(ReNodes.DIFF_AND_DECOMPILE, diff_and_decompile)

    builder.set_entry_point(ReNodes.ANALYZE)
    builder.add_edge(ReNodes.ANALYZE, ReNodes.DIFF_AND_DECOMPILE)
    builder.add_edge(ReNodes.DIFF_AND_DECOMPILE, END)
